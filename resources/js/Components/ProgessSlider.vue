...
<template>
  <div class="w-full max-w-5xl mx-auto text-center mb-4">
        ...
        <template :key="index" v-for="(item, index) in items">
          <TransitionRoot
            ...
            <a :href="item.url" target="_blank">
                <img class="rounded-xl" :src="item.image" width="1024" height="576" :alt="item.title">
    ...
    <!-- Buttons -->
    <div class="max-w-xs sm:max-w-sm md:max-w-3xl mx-auto grid grid-cols-5 gap-2 mt-4">
      <template :key="index" v-for="(item, index) in items">
        <button
        ...
        >
          <span class="text-center flex flex-col items-center" :class="active === index ? '' : 'opacity-50 group-hover:opacity-100 group-focus:opacity-100 transition-opacity'">
           <span class="block relative w-full bg-slate-200 h-1 rounded-full" role="progressbar" :aria-valuenow="active === index ? progress : 0" aria-valuemin="0" aria-valuemax="100">
              <span class="absolute inset-0 bg-red-500 rounded-[inherit]" :style="`${active === index ? `width: ${progress}%` : 'width: 0%'}`"></span>
            </span>
          </span>
        </button>
      </template>
